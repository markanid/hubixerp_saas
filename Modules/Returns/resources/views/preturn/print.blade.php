...
@php
  ...
  $invoiceTitle = $showTax ? 'PURCHASE RETURN TAX INVOICE' : 'PURCHASE RETURN INVOICE';
  $showBatch = $returnDetails->contains(fn ($detail) => !empty($detail->batch_no));
  $leadingColspan = 5 + ($showBatch ? 1 : 0);
  $taxSummaryColspan = 7 + ($showBatch ? 1 : 0);
@endphp

<div class="card card-outline card-primary">
<div class="card-body">

  <a class="btn-back dontprint" href="{{ (string) $return->status === '0' ? route('purchase-returns.showCancelled', $return->pr_id) : route('purchase-returns.show', $return->pr_id) }}">Back</a>
  ...
  <table class="invoice-table">
    ...
    <tr>
      ...
      <td>
        ...
        <b>Purchase Bill No: </b>{{ $return->pr_pvno ?? '-' }}<br><br>
        ...
        <table style="width:100%; border-collapse:collapse; table-layout:fixed;">
          ...
          <tr>
            <td style="padding:8px; border-right:1px solid #00000057;">
              <b>Supplier:</b><br>
              {{ $return->vendor?->cp_name ?? '-' }}<br>
              {{ $return->vendor?->cp_address ?? '-' }}<br>
              Phone: {{ $return->vendor?->cp_phone ?? '-' }}<br>
              {{ $taxNumberLabel }}: {{ $return->vendor?->cp_gst_no ?? '-' }}
            </td>
            <td style="padding:8px;">
              <b>Contact Person:</b><br>
              {{ $return->vendor?->cp_cname ?? '-' }}<br>
              Phone: {{ $return->vendor?->cp_cphone ?? '-' }}<br>
              Email: {{ $return->vendor?->cp_email ?? '-' }}
            </td>
          </tr>
        </table>
      </td>
  ...
  <table class="bordered" style="margin-top:5px;">
    <tr>
      <td width="65%">
        <b>Declaration:</b>
        We declare that this purchase return records goods returned to the supplier and all particulars are true and correct.
      </td>
  ...
  <p align="center">This is a Computer Generated Purchase Return Invoice<br>Thank You</p>

</div>
</div>

@include('partials.local-print-script', ['printSetting' => $printSetting ?? null, 'documentType' => 'purchase_return', 'documentId' => $return->pr_id])
</body>
</html>